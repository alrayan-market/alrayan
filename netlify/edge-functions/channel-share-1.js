...
export default async (request, context) => {
  ...
  if((!vid&&!poll&&!ad)||!BOT.test(ua)) return context.next();
  let title="قناة الريان", desc="منصة الريان — السوق السوداني الموثوق", image=SITE_LOGO;
  try{
    if(ad){
      const d=await fireGet("ads",ad);
      const f=d&&d.fields||{};
